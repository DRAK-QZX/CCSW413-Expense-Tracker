...
class Analytics extends JFrame {
    ...
    Analytics(String username) {
        ...
        JButton exportBtn = new JButton("Export CSV Report");
        exportBtn.setBackground(new Color(0, 153, 102));
        exportBtn.setForeground(Color.WHITE);
        exportBtn.setFocusPainted(false);
        exportBtn.setBorderPainted(false);
        exportBtn.setOpaque(true);
        exportBtn.setFont(f1);

        /*
         * Stage 2: Adapter Pattern
         * The GUI uses the ReportExporter interface.
         * CSVExporterAdapter adapts OldCSVExporter so it can work with exportReport().
         */
        exportBtn.addActionListener(a -> {
            try {
                OldCSVExporter oldExporter = new OldCSVExporter();

                ReportExporter exporter = new CSVExporterAdapter(oldExporter);

                exporter.exportReport();

                JOptionPane.showMessageDialog(
                        null,
                        "Report exported successfully using Adapter Pattern."
                );

            } catch (Exception e) {
                JOptionPane.showMessageDialog(
                        null,
                        "Error while exporting report: " + e.getMessage()
                );
                e.printStackTrace();
            }
        });
        ...
        try {
            ...
            try (PreparedStatement pst = con.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    String legendType = type.substring(0, 1).toUpperCase()
                            + type.substring(1).toLowerCase();

                    dataset.setValue(total, legendType, monthName);
                }
            }

        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Chart Error: " + e.getMessage());
        }
    }
